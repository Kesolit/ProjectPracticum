...
import { loginUser, API_BASE_URL } from '../../api/api'
import './Login.css'
import { validateEmail, validatePassword } from '../../utils/validation'
import { LoginMobile } from './LoginMobile' // Импортируем мобильную версию
...
const Login = () => {
  ...
  // Добавляем стейт для отслеживания мобильной версии экрана
  ...
  const handleSubmit = async (e?: React.FormEvent, mobileData?: typeof formData) => {
    if (e) e.preventDefault();
    
    // Поддерживаем данные как из десктопной формы, так и переданные из мобильной
    const currentData = mobileData || formData;
    
    const emailErr = validateEmail(currentData.email);
    const passErr = validatePassword(currentData.password);
    ...
    setIsLoading(true);
    try {
      const result = await loginUser({ email: currentData.email, password: currentData.password });
  ...
  const handleGoogleLogin = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/user/google-login`);
      const result = await response.json();
      if (result.success && result.data?.url) {
        window.location.href = result.data.url;
      } else {
        setServerError('Не удалось получить ссылку для входа через Google');
      }
    } catch (err) {
      setServerError('Ошибка при попытке входа через Google');
    }
  };

  // ЕСЛИ МОБИЛКА: рендерим изолированный компонент и передаем нужные пропсы и стейты
  if (isMobile) {
    return (
      <LoginMobile 
  ...
  // ЕСЛИ ДЕСКТОП: оставляем твой исходный UI без изменений
  return (
    <div className="login-page">
      <header className="login-header">
        <div className="login-logo">
          <img src={logo} alt="dev/folio" className="login-logo-icon" />
          <span className="login-logo-text">dev/folio</span>
        </div>
      </header>
      ...
      <div className="login-content">
        <div className="login-container">
          ...
          <p className="login-subtitle">
            Введите свои данные, чтобы получить доступ к своему личному кабинету.
          </p>
          ...
          <form onSubmit={(e) => handleSubmit(e)} autoComplete="off">
            ...
            <div className="login-options-row">
              ...
              <a href="#" className="login-forgot-link">Забыли пароль?</a>
            </div>
          ...
          <div className="login-social-section">
            <div className="login-divider">Или войдите с помощью</div>
